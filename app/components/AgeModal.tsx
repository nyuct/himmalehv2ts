...
const AgeModal: React.FC = () => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (
      today < birthDateObj ||
      age < 25 ||
      (age === 25 && (today.getMonth() < birthDateObj.getMonth() ||
  ...
  return !isSubmit ? null : (
    <section className={`modal-age-popup  ${isVerified ? 'hide' : 'active'}  `} id="modalAgePopup">
      <div className={`showAgeForm ${isSubmit ? '' : 'active'}`}>
        ...
        <form id="ageForm" onSubmit={handleSubmit}>
          ...
          <button id="ageButton" type="submit">Enter</button>
          {error && <div id="ageError" style={{ color: 'red' }}>{error}</div>}
        </form>
        <h6>BY ENTERING THIS YOU ARE AGREEING TO THE TERMS OF USE AND PRIVACY POLICY.</h6>
      </div>
      <div className={`showAgeMsg ${isSubmit ? 'active' : ''}`}>
        <h3 className="age-heading">
          YOU ARE APPROXIMATELY 50 MILLION YEARS <br /> YOUNGER TO THE PLACE WHERE WE ARE TAKING YOU
        </h3>
